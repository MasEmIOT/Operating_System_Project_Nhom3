...
static void on_display_mode_toggled(GtkCheckButton *check, gpointer data) {
    if (gtk_check_button_get_active(check)) {
        ...
        const char *title = app.display_mode == 0 ?
            "Chart ACC (last ~20s)" : "Chart GYRO (last ~20s)";
    ...
    if (app.simulation_mode) {
        ...
        if (read(app.fd, &app.data, sizeof(app.data)) < 0) {
            gtk_label_set_text(GTK_LABEL(app.label_status),
                               "[Error] Cannot read /dev/mpu6050");
            app.running = 0;
            return TRUE;
        }
    }

    /* ===== ALWAYS LOG BOTH ACC & GYRO ===== */

    // ACC
    app.history_acc_x[app.acc_index] = app.data.accel_x;
    app.history_acc_y[app.acc_index] = app.data.accel_y;
    app.history_acc_z[app.acc_index] = app.data.accel_z;
    app.acc_index = (app.acc_index + 1) % 100;
    if (app.acc_count < 100) app.acc_count++;

    // GYRO
    app.history_gyro_x[app.gyro_index] = app.data.gyro_x;
    app.history_gyro_y[app.gyro_index] = app.data.gyro_y;
    app.history_gyro_z[app.gyro_index] = app.data.gyro_z;
    app.gyro_index = (app.gyro_index + 1) % 100;
    if (app.gyro_count < 100) app.gyro_count++;
...
// GRAPH 
static void draw_chart(GtkDrawingArea *area, cairo_t *cr,
    ...
    if (app.display_mode == 0) {
        hx = app.history_acc_x;
        hy = app.history_acc_y;
        hz = app.history_acc_z;
        count = app.acc_count;
        index = app.acc_index;
    } else {
        hx = app.history_gyro_x;
        hy = app.history_gyro_y;
        hz = app.history_gyro_z;
        count = app.gyro_count;
        index = app.gyro_index;
    }
    ...
    for (int i = 0; i < count; i++) {
        int idx = (index - count + i + 100) % 100;
        double x = i * step_x;
        double y = height / 2 - hx[idx] * scale_y;
    ...
    for (int i = 0; i < count; i++) {
        int idx = (index - count + i + 100) % 100;
        double x = i * step_x;
        double y = height / 2 - hy[idx] * scale_y;
        if (i == 0) cairo_move_to(cr, x, y);
        else cairo_line_to(cr, x, y);
    }
    cairo_stroke(cr);
    ...
    for (int i = 0; i < count; i++) {
        int idx = (index - count + i + 100) % 100;
        double x = i * step_x;
        double y = height / 2 - hz[idx] * scale_y;
        if (i == 0) cairo_move_to(cr, x, y);
        else cairo_line_to(cr, x, y);
    }
    cairo_stroke(cr);
...
static void on_start_clicked(GtkButton *button, gpointer data) {
    ...
    if (!app.simulation_mode) {
        ...
        if (app.fd < 0) {
            gtk_label_set_text(GTK_LABEL(app.label_status),
                               "[Error] /dev/mpu6050 not found");
            return;
        }
    }

    app.running = 1;
    app.acc_index = app.acc_count = 0;
    app.gyro_index = app.gyro_count = 0;

    memset(app.history_acc_x, 0, sizeof(app.history_acc_x));
    memset(app.history_acc_y, 0, sizeof(app.history_acc_y));
    memset(app.history_acc_z, 0, sizeof(app.history_acc_z));
    memset(app.history_gyro_x, 0, sizeof(app.history_gyro_x));
    memset(app.history_gyro_y, 0, sizeof(app.history_gyro_y));
    memset(app.history_gyro_z, 0, sizeof(app.history_gyro_z));
...
static void activate(GtkApplication *gtk_app, gpointer user_data) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(app.window), 780, 650);
